...
// write your for loop here to iterate over the "array" of html elements
for (let i = 0; i <= carnivalRecipes.length - 1; i++) {
  //   title info
  let title = carnivalRecipes[i].title;
  // console.log($title);

  // image info
  let src = carnivalRecipes[i].image;
  // console.log(foundRecipes[i]);

  // // preview text info
  // let $previewText = $(recipeData).find("div > .card__imageContainer > a");
  let previewText = carnivalRecipes[i].previewText;

  let link = carnivalRecipes[i].link
